...
- 修复网址导航打开后白屏的问题，并优化网址导航列表、分类和提交交互的卡顿。
- 数据管理切换数据目录后会写入稳定指针，重启后继续使用新目录；迁移使用移动语义，跨盘时自动兜底处理。
- 周/月/年心迹说明改为符合实际逻辑：周汇总日心迹、月汇总周心迹、年汇总月心迹。
- 软件默认窗口调整为 `1280×840`，首次使用侧边栏默认展开，并提示支持折叠和拖拽排序。
- 暗色模式下心迹内容对比度增强，阅读更清楚。
- AI 生成的心迹海报会保存到本地，降低远程链接失效风险。
- 心迹海报查看器优先加载缩略预览，减少打开大图时的卡顿。
- 网址导航重复提交会检查未通过、审核中和已通过状态，同一网址仅允许提交一次。
- 修复“我的数资”中网址分享统计兼容问题。
- 签到积分首次打开会自动签到，当天再次打开不重复签到。
- 窗口右上角置顶图钉启用后颜色更明显。

## 1.2.5 更新重点

- 修复剪切板文件和目录再次复制失败的问题，目录可直接打开定位。
- 网址导航自动隐藏被拒绝或删除的内容，推荐网址在聚合首页置顶并显示固定角标。
- 通知设置移除云空间相关入口与提示。
- 心迹按“日 → 周 → 月 → 年”逐级汇总，修复日心迹未按设置时间自动生成的问题。
- 长心迹海报不再被截断，支持在固定预览窗口内上下滚动查看。
- 备忘录四象限新建窗口改为四格彩色选择，分类更直观。
- 修复“我的数资”中网址分享统计不准确的问题。
- 会员等级与权益弹窗只展示实际支持的客户端功能，不再显示云空间字段。
