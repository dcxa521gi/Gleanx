...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-2.0.0-7c3aed" alt="Gleanx 2.0.0">
</p>
...
本次 GitHub 公共版本合并 1.9.0 与 2.0.0 的 Windows 客户端更新；1.9.0 未单独发布到 GitHub，相关新增、优化和修复统一归入 2.0.0。以下只说明客户端能力。

- 新增天气中心：支持自动定位、IP 回退、手动选择城市/区县、天气预警、未来天气、空气质量、天气指数、分钟降水、天文和时光机等信息。
- 天气显示更准确：支持显示定位来源和手动纠偏，分类保持单行展示，天气指数内容区可独立滚动。
- 剪切板新增收藏管理：支持右键收藏、取消收藏、收藏数量显示、收藏弹窗查看、搜索、复制和定位原记录。
- 密钥分类更安全：默认仅显示前后各 3 位，中间内容隐藏，点击眼睛按钮后临时查看完整内容。
- 清空历史更稳妥：已置顶、已收藏、已设置快捷键的剪切板内容不会被直接清空。
- 新增赞助用户自动签到：有效赞助等级用户可在软件运行时按自然日自动签到领取积分。
- 新增端到端加密云备份：支持定时备份、启动备份、退出前备份、配置覆盖和剪切板文字增量去重。
- 个人中心新增云备份管理：可查看、解密、恢复和删除备份记录。
- 开机启动更符合桌面习惯：电脑开机后默认静默驻留托盘，不再自动弹出主窗口。
- 定时管理新增关机任务：支持提前倒计时、取消和延迟关机。
- 积分中心新增积分购买：支持套餐、自定义积分数、微信/支付宝支付和自动到账。
- 性能继续优化：OCR 资源按需加载并在空闲后释放，减少启动占用、空闲内存和异常重试造成的卡顿。
