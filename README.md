...
> 让复制、整理、回看、提醒和生成都集中在一个 Windows 客户端里。
...
- 修复注册账号时误提示“缺少访问令牌”的问题。
- 优化剪切板图片预览：预览框不随缩放变化，图片居中显示并支持拖拽。
- 优化心迹海报预览：海报缩放不改变预览框大小。
- 心迹海报默认不带二维码；点击“添加二维码”后，二维码默认出现在底部，并支持拖拽到任意位置。
- 定时任务点击“添加”后改为弹窗添加。
...
- 经常处理大量复制内容、截图、链接和文件的 Windows 用户。
